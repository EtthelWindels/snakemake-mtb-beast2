...
get_extended_log <- function(path_to_logfile) {
  ...
  log$migr <- log$migrationMatrix_0
  log$latent_period <- log$inverseMigrationRate
...
Re_TG <- plot_Re("The Gambia", lineage_labels=c("L2","L4","L6"), lineage_colours=c(L6_col,L4_col,L2_col)) +
  theme(plot.margin = margin(t=0.5, b=0.5, unit="cm"))
Re_VN <- plot_Re("Vietnam", x_max=3, lineage_labels=c("L1","L2","L4"), lineage_colours=c(L4_col,L2_col,L1_col)) +
...
uninfperiod_TG <- plot_latentperiod("The Gambia", x_max=6, lineage_labels=c("L2","L4","L6"), lineage_colours=c(L6_col,L4_col,L2_col)) +
  theme(plot.margin = margin(b=0.5, unit="cm")) 
uninfperiod_VN <- plot_latentperiod("Vietnam", x_max=12, lineage_labels=c("L1","L2","L4"), lineage_colours=c(L4_col,L2_col,L1_col)) +
...
ggsave("bdmm_inverse.png", plot=plot_all, path=dirname(snakemake@output[["png"]]), width=250, height=300, units="mm", bg='white')
ggsave("bdmm_inverse.svg", plot=plot_all, path=dirname(snakemake@output[["svg"]]), width=250, height=300, units="mm", bg='white')