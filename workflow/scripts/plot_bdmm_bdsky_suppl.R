...
get_extended_log <- function(path_to_logfile) {
  log <- load_trajectories(path_to_logfile, burninFrac=0) %>%
    as.data.frame()
  log$bUR <- log$becomeUninfectiousRate.4
  log$inf_period <- 1/log$bUR
  log$lambda <- log$bUR 
  log$migr <- log$rateMatrix.2
...
uninfperiod_Ma <- plot_latentperiod("Malawi", lineage_labels=c("L1","L2","L3","L4"), lineage_colours=c(L4_col,L3_col,L2_col,L1_col)) +
  theme(plot.margin = margin(t=0.5, b=0.5, l=0.5, unit="cm"))
uninfperiod_Ta <- plot_latentperiod("Tanzania", lineage_labels=c("L1","L2","L3","L4"), lineage_colours=c(L4_col,L3_col,L2_col,L1_col)) +
  theme(plot.margin = margin(t=0.5, b=0.5, unit="cm"))
uninfperiod_TG <- plot_latentperiod("The Gambia", x_max=4, lineage_labels=c("L2","L4","L6"), lineage_colours=c(L6_col,L4_col,L2_col)) +
  theme(plot.margin = margin(t=0.5, b=0.5, unit="cm"))
uninfperiod_VN <- plot_latentperiod("Vietnam", x_max=4, lineage_labels=c("L1","L2","L4"), lineage_colours=c(L4_col,L2_col,L1_col)) +
...
transrate_Ta <- plot_transrate("Tanzania", lineage_labels=c("L1","L2","L3","L4"), lineage_colours=c(L4_col,L3_col,L2_col,L1_col)) +
  theme(plot.margin = margin(b=0.5, unit="cm"))
transrate_TG <- plot_transrate("The Gambia", lineage_labels=c("L2","L4","L6"), lineage_colours=c(L6_col,L4_col,L2_col)) +
  theme(plot.margin = margin(b=0.5, unit="cm")) 
transrate_VN <- plot_transrate("Vietnam", lineage_labels=c("L1","L2","L4"), lineage_colours=c(L4_col,L2_col,L1_col)) +
  theme(plot.margin = margin(b=0.5, r=0.5, unit="cm"))

# Combined plot

plots_col1 <- align_plots(uninfperiod_Ma, distr_Ma, infperiod_Ma, transrate_Ma, align='v', axis='l')
plots_col2 <- align_plots(uninfperiod_Ta, distr_Ta, infperiod_Ta, transrate_Ta, align='v', axis='l')
plots_col3 <- align_plots(uninfperiod_TG, distr_TG, infperiod_TG, transrate_TG, align='v', axis='l')
plots_col4 <- align_plots(uninfperiod_VN, distr_VN, infperiod_VN, transrate_VN, align='v', axis='l')
...
plot_all <- plot_grid(plot_a, plot_b, plot_c, plot_d, ncol=1, align='v', axis='l', rel_heights=c(1.3,1,1,1))



# Save output plots -------------------------------------------------------

ggsave("bdmm_bdsky_fixedRe.png", plot=plot_all, path=dirname(snakemake@output[["png"]]), width=250, height=250, units="mm", bg='white')
ggsave("bdmm_bdsky_fixedRe.svg", plot=plot_all, path=dirname(snakemake@output[["svg"]]), width=250, height=250, units="mm", bg='white')